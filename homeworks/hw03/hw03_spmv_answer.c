              ...
              int *res)
{
	...
	for (i = 0; i < row; ++i) {
		// Initialize cnt and being/end which contain the range in which the rows data are contained
		cnt = 0;
		begin = row_ptr[i];
		end = row_ptr[i+1];
		// For each coloumn multiply val to vector and add to cnt var
		for (j = begin; j < end; ++j) {
			cnt += val[j] * vec[col_ind[j]];
		}
		// Set res[i] to count. write to memory outside of for loop is better than in
		res[i] = cnt;
	}
}
                        ...
                        int **coo_row, int **coo_col, int **coo_val)
{
	...
	// malloc vars are changed and malloc size, but code copied from above
    coo_ptr_ = (int*) malloc(sizeof(int) * row_ptr[row]);
    coo_ind_ = (int*) malloc(sizeof(int) * row_ptr[row]); 
    if(coo_ind_ == NULL) {
        fprintf(stderr, "Malloc error for coo_ind_\n");
        exit(EXIT_FAILURE);
    }
    coo_val_= (int*) malloc(sizeof(int) * row_ptr[row]); 
    if(coo_val_ == NULL) {
	...
	for(i = 0; i < row; ++i) {
		// set same begin and end vals
		begin = row_ptr[i];
		end = row_ptr[i+1];
		// Loop through and set each var to its corresponding
		for(j = begin; j < end; ++j) {
			coo_ptr_[cnt] = i;
			coo_ind_[cnt] = col_ind[cnt];
			coo_val_[cnt] = val[cnt];
			// increment cnt, which represents nnz
			++cnt;
		}
	}
              ...
              int *coo_val, int *vec, int *res)
{
	...
	int i;
	// Loop through each nnz, add to corresponding row in res vector
	for(i = 0; i < nnz; i++) {
